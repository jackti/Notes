...
![CNN](assets/CNN.jpg){ width="95%" height="95%" }


## CNN前向传播算法
卷积神经网络对比全连接神经网络，其保留了图像像素的空间排列信息。卷积操作将一个全连接层稀疏连接和共享参数。如图展示了全连接层到卷积层的转换过程。

![CNN](assets/CNN5.png){ width="65%" height="65%" }
...
![CNN](assets/CNN2.png){ width="85%" height="85%" }
...
![CNN](assets/pooling.png){ width=75%" height="75%" }
...
### 已知池化层误差$\delta^l$,求$l-1$层误差$\delta^{l-1}$
...
![CNN](assets/CNN3.png){ width="95%" height="95%" }

将上面的卷积过程展开，并用神经网络的形式描述——权重上的标注是其在卷积核中的位置,相同的权重用同一种颜色表示.

![CNN](assets/CNN4.png){ width="50%" height="50%" }
...
& \xrightarrow[m+a=i]{n+b=j} \sum_{m} \sum_{n} \delta_{m, n}^{l} \boldsymbol{W}_{i-m, j-n}^{l} \sigma^{\prime}\left(\boldsymbol{z}_{i, j}^{l-1}\right)
\end{align}
$$
...
& \xrightarrow[a=i]{b=j} \sum_{m} \sum_{n} \delta_{m, n}^{l} \boldsymbol{a}_{m+i, n+j}^{l-1}
\tag{CNN-3}
\end{align}
$$
